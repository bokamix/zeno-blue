...
"""Restart a deployed app without changing its URL."""
...
def main():
    parser = argparse.ArgumentParser(description="Restart a deployed app.")
    parser.add_argument("--app-id", required=True, help="The app_id to restart")
    args = parser.parse_args()

    app_id = args.app_id

    try:
        response = httpx.post(f"{API_BASE}/apps/{app_id}/__restart", timeout=30)
        ...
        print(json.dumps({
            ...
            "note": "App has been restarted. It may take a moment to become available."
        }, indent=2))
        ...
        print(json.dumps({"error": f"Failed to restart app: {str(e)}"}))
        sys.exit(1)
